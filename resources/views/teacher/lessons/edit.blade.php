            ...
            <form id="lesson-form" action="{{ route('teacher.lessons.update', [$course, $lesson]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div id="step-media" class="step-content hidden">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 rounded-xl p-4 sm:p-6 border border-blue-200 dark:border-gray-600 mb-4">
                        ...
                        <input type="hidden" id="media_data" name="media_data" value="{{ old('media_data', json_encode($lesson->media_data ?? [])) }}">
                        <!-- Dropzone -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addMediaItem(file, type = null, url = null, title = null, description = null, existingId = null) {
        const mediaType = type || getMediaType(file);
        const itemId = existingId || 'media-' + (++mediaCounter);
        ...
        details.textContent = file ? `${(file.size / 1024 / 1024).toFixed(2)} MB • ${mediaType.toUpperCase()}` : (url || 'Existing Media');
        ...
        updateMediaData();
        toastr.success(`${file ? file.name : (url ? 'Media' : 'Existing media')} added successfully!`);
    ...
    function renderMediaItems() {
        ...
        attachedMedia.forEach(item => {
            ...
            const details = document.createElement('div');
            details.className = 'text-sm text-gray-500';
            details.textContent = file ? `${(file.size / 1024 / 1024).toFixed(2)} MB • ${mediaType.toUpperCase()}` : (item.url || 'Existing Media');
            ...
            if (mediaType === 'image' && (file || item.url)) {
                const preview = document.createElement('div');
                preview.className = 'mt-3';
                const img = document.createElement('img');
                img.src = file ? URL.createObjectURL(file) : item.url;
                img.className = 'max-w-xs rounded-lg shadow-lg';
                preview.appendChild(img);
                mediaItem.appendChild(preview);
            }
