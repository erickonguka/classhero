...
class DiscussionController extends Controller
{
    ...
    public function store(Request $request, Lesson $lesson)
    {
        ...
        // Check comment limit (5 per lesson per user)
        ...
        // Check banned words
        $bannedWords = ['spam', 'scam', 'fake', 'stupid', 'idiot', 'hate', 'kill', 'die', 'damn', 'hell'];
        $content = strtolower($request->content);
        foreach ($bannedWords as $word) {
            if (strpos($content, $word) !== false) {
                return response()->json(['error' => 'Your comment contains inappropriate content'], 422);
            }
        }
